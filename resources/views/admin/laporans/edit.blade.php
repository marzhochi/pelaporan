    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.laporans.update", [$laporan->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('deskripsi') ? 'is-invalid' : '' }}" type="text" name="deskripsi" id="deskripsi" value="{{ old('deskripsi', $laporan->deskripsi) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lokasi') ? 'is-invalid' : '' }}" name="lokasi_id" id="lokasi_id">
                    @foreach($lokasis as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lokasi_id') ? old('lokasi_id') : $laporan->lokasi->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tugas') ? 'is-invalid' : '' }}" name="tugas_id" id="tugas_id">
                    @foreach($tugas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('tugas_id') ? old('tugas_id') : $laporan->tugas->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('tugas'))
                    <div class="invalid-feedback">
