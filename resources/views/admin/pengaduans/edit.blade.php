...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pengaduan.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pengaduans.update", [$pengaduan->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nama_lengkap">{{ trans('cruds.pengaduan.fields.nama_lengkap') }}</label>
                <input class="form-control {{ $errors->has('nama_lengkap') ? 'is-invalid' : '' }}" type="text" name="nama_lengkap" id="nama_lengkap" value="{{ old('nama_lengkap', $pengaduan->nama_lengkap) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_telepon') ? 'is-invalid' : '' }}" type="text" name="no_telepon" id="no_telepon" value="{{ old('no_telepon', $pengaduan->no_telepon) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('judul_pengaduan') ? 'is-invalid' : '' }}" type="text" name="judul_pengaduan" id="judul_pengaduan" value="{{ old('judul_pengaduan', $pengaduan->judul_pengaduan) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('keterangan') ? 'is-invalid' : '' }}" type="text" name="keterangan" id="keterangan" value="{{ old('keterangan', $pengaduan->keterangan) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('latlang') ? 'is-invalid' : '' }}" type="text" name="latlang" id="latlang" value="{{ old('latlang', $pengaduan->latlang) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lokasi') ? 'is-invalid' : '' }}" name="lokasi_id" id="lokasi_id">
                    @foreach($lokasis as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lokasi_id') ? old('lokasi_id') : $pengaduan->lokasi->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
...
Dropzone.options.fotoDropzone = {
    url: '{{ route('admin.pengaduans.storeMedia') }}',
    maxFilesize: 2, // MB
